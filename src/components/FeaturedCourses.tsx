...
interface Course{ // this is the custom data type that will be used for verification in the typeScript format if any of the below fields is unavailable then compiler error dega
    id:number,
    title:string,
    slug:string,
    description:string,
...
export default function FeaturedCourses(){
    const featuredCourses = courseData.courses.filter((course:Course)=> course.isFeatured===true) // ab yaha humme courses ka type define karna hota hai thats the work that we have to do in typescript
            ...
            <div className='mt-10'>
                <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 justify-center'>
                    {featuredCourses.map((courses:Course)=>(
                       <div key={courses.id} className='felx justify-center'>
                            <BackgroundGradient
